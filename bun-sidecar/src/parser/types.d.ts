declare module CoH {
	...
	type EventDefaultData = { line: string };
	type EventData<T = Record<string, any>> = T & EventDefaultData;
	...
	type LogEvents = {
		...
		'LOG:LOBBY:POPULATING:PLAYER': (index, playerId, type, team, race) => void;
	...
	type LobbyPopulatedSteamId = (steamId: bigint) => void;
	type LobbyPopulatedPlayerCount = (playerCount: number) => void;
	type LobbyPopulatedPlayer = (player: IPlayer) => void;
	type LobbyPopulatedMap = (map: string) => void;
