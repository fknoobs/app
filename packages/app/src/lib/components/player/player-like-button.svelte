<script lang="ts">
	...
	let { steamId, likeCount = 0, onCountChange }: Props = $props();
	const { t } = useI18n();
	const myVote = resource(
		() => steamId,
		(id) => app.database.playerSocial.getMyVote(id)
	);
	...
	async function setVote(value: 1 | -1) {
		...
		voting = true;
		try {
			const result = await app.database.playerSocial.setPlayerVote(steamId, value);
			vote = result.vote;
