...
import type { ReputationTypesResponse, ReputationTypesTriggerOptions } from './types';

export type ReputationType = ReputationTypesResponse;
export type ReputationTrigger = ReputationTypesTriggerOptions;
...
export async function listReputationTypes(): Promise<ReputationType[]> {
	return pocketbase.collection('reputation_types').getFullList<ReputationTypesResponse>({
		sort: 'sort,name',
		fetch
	});
}
