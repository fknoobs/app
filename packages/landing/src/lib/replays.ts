...
export type HistorySortField = 'createdAt' | 'likeCount' | 'downloadCount';
...
const SORT_FIELDS = new Set<HistorySortField>(['createdAt', 'likeCount', 'downloadCount']);
...
export function buildMatchHistoryUrl(query: ReplaysQuery): string {
	const params = new URLSearchParams({
		scope: 'community',
		page: String(query.page),
		perPage: String(REPLAYS_PER_PAGE)
	});
...
export function formatMatchDate(value: string | undefined): string {
	if (!value) return '—';
